...
const grid = {
  ...
  draw: function (bgColor) {
    ...
    for (let x = 0; x < this.ticksHorizontal; x++) {
      for (let y = 0; y < this.ticksVertical; y++) {
...
const = Label = function () {
  return {
    ...
    setKeyColor: function (c) {
      this.keyColor = c;
      return this;
    },
    setValueColor: function (c) {
      this.valueColor = c;
      return this;
    },
    draw: function () {
...
function requestClusterStateInterval() {
  ...
  for (let port = 9551; port <= 9559; port++) {
...
function clusterStateScanForOfflineNodes(nodes) {
  ...
  for (let n = 0; n < 9; n++) {
    if (time - nodes[n].time > 3000) {
      // node is offline if no update for over 3 seconds
...
function clusterStateUpdateNode(clusterStateFromNode) {
  ...
  for (let n = 0; n < clusterStateFromNode.nodes.length; n++) {
